...
Combatant *GetSmallestCooldownCombatant(std::vector<Combatant *> combatants)
{
	auto min_cd_combatant =
	    std::min_element(combatants.begin(), combatants.end(), [](const Combatant *a, const Combatant *b) {
		    return std::min(a->cooldown_,
		                    (a->state_reset_countdown_ < 0 ? a->cooldown_ + 1 : a->state_reset_countdown_))
		           < std::min(b->cooldown_,
		                      (b->state_reset_countdown_ < 0 ? b->cooldown_ + 1 : b->state_reset_countdown_));
	    });
...
void Combat::SetInitialCooldowns()
{
	...
	for (Combatant *combatant : all_combatants) {
		if (diff == 0) {
			...
			combatant->cooldown_ =
			    MAX_INIT_CD - (int)(MAX_INIT_CD * ((float)(combatant->agility() - min_max.first) / diff));
		}
...
void Combat::ProgressToNextAction()
{
	...
	int cd_player = std::min(min_cd_player->cooldown_,
	                         (min_cd_player->state_reset_countdown_ < 0 ? min_cd_player->cooldown_ + 1
	                                                                    : min_cd_player->state_reset_countdown_));

	Combatant *min_cd_enemy = GetSmallestCooldownCombatant(living_enemy_combatants);
	int cd_enemy = std::min(min_cd_enemy->cooldown_,
	                        (min_cd_enemy->state_reset_countdown_ < 0 ? min_cd_enemy->cooldown_ + 1
	                                                                  : min_cd_enemy->state_reset_countdown_));

	int cooldown_progression = std::min(cd_player, cd_enemy);

	Combatant *min_cd_combatant;
	if (cd_player == cd_enemy && cd_player == min_cd_player->cooldown_
	    && cd_enemy == min_cd_enemy->state_reset_countdown_) {
	...
	if (min_cd_combatant->state_reset_countdown_ < 0
	    || min_cd_combatant->state_reset_countdown_ > min_cd_combatant->cooldown_) {
...
void Combat::PerformEnemyTurn()
{
	...
	if (action_number < attacks_size) {
		...
		if (active_turn_chosen_attack_->target_type == AttackTargetType::single) {
			display_text_ = active_combatant_->PerformAttack(
			    *active_turn_chosen_attack_,
			    {living_player_combatants.at(std::rand() % living_player_combatants.size())});
		} else {
		...
		case AbilityTargetType::self:
			targets = {active_combatant_};
			break;
		case AbilityTargetType::enemy_single:
			targets = {living_player_combatants.at(std::rand() % living_player_combatants.size())};
			break;
		case AbilityTargetType::enemy_multi:
			targets = living_player_combatants;
			break;
		case AbilityTargetType::team_single:
			targets = {living_enemy_combatants.at(std::rand() % living_enemy_combatants.size())};
			break;
		case AbilityTargetType::team_multi:
			targets = living_enemy_combatants;
			break;
		case AbilityTargetType::all:
			targets = AllLivingCombatants();
		}
...
void Combat::SetActionAndProgress(Attack *attack, Ability *ability)
{
	if (attack != NULL) {
		...
		case AbilityTargetType::self:
			display_text_ = this->active_combatant_->UseAbility(*ability, {active_combatant_});
			this->state_ = CombatState::action_display;
			break;
		case AbilityTargetType::enemy_single:
			this->active_turn_targets_ = LivingEnemyCombatants();
			...
			break;
		case AbilityTargetType::enemy_multi:
			display_text_ = this->active_combatant_->UseAbility(*ability, LivingEnemyCombatants());
			this->state_ = CombatState::action_display;
			break;
		case AbilityTargetType::team_single:
			this->active_turn_targets_ = LivingPlayerCombatants();
			this->state_ = CombatState::ability_target_selection;
			break;
		case AbilityTargetType::team_multi:
			display_text_ = this->active_combatant_->UseAbility(*ability, LivingPlayerCombatants());
			this->state_ = CombatState::action_display;
			break;
		case AbilityTargetType::all:
			display_text_ = this->active_combatant_->UseAbility(*ability, AllLivingCombatants());
			this->state_ = CombatState::action_display;
			break;
		}
	}
...
void Combat::Progress(Attack *attack, Ability *ability, Combatant *target)
{
	...
	case CombatState::action_display:
		ProgressToNextAction();
		break;
	case CombatState::losing_screen: /*Cleanup;*/
		break;
	...
	case CombatState::exp_gain_display:
		...
		break;
	case CombatState::loot_display: /*Cleanup;*/
		break;
	case CombatState::escape_screen: /*Cleanup;*/
		break;
	}
}