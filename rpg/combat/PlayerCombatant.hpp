...
class PlayerCombatant : public Combatant {
  public:
	...
	PlayerCombatant(const std::string &name, const std::string &sprite_name, int level, int max_hp, int agility,
	                int strength, int defense, int dexterity, int perception, int luck);
	~PlayerCombatant();
	...
	void serialize(Archive &archive)
	{
		archive(CEREAL_NVP(exp_), CEREAL_NVP(base_attacks_), CEREAL_NVP(base_abilities_), CEREAL_NVP(name_),
		        CEREAL_NVP(sprite_name_), CEREAL_NVP(level_), CEREAL_NVP(max_hp_), CEREAL_NVP(hp_),
		        CEREAL_NVP(agility_), CEREAL_NVP(strength_), CEREAL_NVP(defense_), CEREAL_NVP(dexterity_),
		        CEREAL_NVP(perception_), CEREAL_NVP(luck_));
	}
  ...
  private:
	...
	void ChooseAbilityTargetAndUseAbility(Ability ability, const std::vector<Combatant *> player_combatants,
