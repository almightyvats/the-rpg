...
#define SWORD_ATTACK_SWORD_SWING \
	{ \
		"Sword Swing", AttackType::melee, AttackTargetType::single, (int)(0.6 * level + 3), 0, 0.85, 0.3, 1.5, 6, \
		    AttackEffect::none \
	}
#define SWORD_ATTACK_POWER_STRIKE \
	{ \
		"Power Strike", AttackType::melee, AttackTargetType::single, (int)(1.5 * level + 1), 0, 0.75, 0.1, 2.0, 12, \
		    AttackEffect::slow \
	}
#define SWORD_ATTACK_QUICK_STAB \
	{ \
		"Quick Stab", AttackType::melee, AttackTargetType::single, (int)(0.4 * level + 1), 0, 0.85, 0.0, 0.0, 3, \
		    AttackEffect::armor_breaking \
	}
#define SWORD_ATTACK_RAGE_BLADE \
	{ \
		"Rageblade", AttackType::melee, AttackTargetType::multi, (int)(0.4 * level + 1), 0, 0.6, 0.3, 2.0, 9, \
		    AttackEffect::none \
	}
#define SWORD_ATTACK_RISKY_STRIKE \
	{ \
		"Risky Strike", AttackType::melee, AttackTargetType::multi, (int)(1.5 * level + 1), 0, 0.3, 0.8, 3.0, 15, \
		    AttackEffect::none \
	}
...
#define AXE_ATTACK_AXE_SWING \
	{ \
		"Axe Swing", AttackType::melee, AttackTargetType::single, (int)(0.8 * level + 1), 0, 0.9, 0.2, 2.0, 8, \
		    AttackEffect::none \
	}
#define AXE_ATTACK_HEAD_CRUSHER \
	{ \
		"Head Crusher", AttackType::melee, AttackTargetType::single, (int)(1.6 * level + 1), 0, 0.8, 0.0, 1.0, 13, \
		    AttackEffect::armor_breaking \
	}
#define AXE_ATTACK_ANKLE_BREAKER \
	{ \
		"Ankle Breaker", AttackType::melee, AttackTargetType::single, (int)(1.2 * level + 1), 0, 0.9, 0.0, 1.0, 10, \
		    AttackEffect::slow \
	}
#define AXE_ATTACK_SPINNING_AXE \
	{ \
		"Spinning Axe", AttackType::melee, AttackTargetType::multi, (int)(0.6 * level + 1), 0, 0.6, 0.2, 2.0, 8, \
		    AttackEffect::none \
	}
#define AXE_ATTACK_AXE_TOSS \
	{ \
		"Axe Toss", AttackType::ranged, AttackTargetType::single, (int)(1.5 * level + 1), (3 * level), 0.7, 0.1, 3.0, \
...
#define BOW_ATTACK_ARROW_SHOT \
	{ \
		"Arrow Shot", AttackType::ranged, AttackTargetType::single, (int)(0.7 * level + 3), penetration, 0.8, 0.05, \
		    3.0, 4, AttackEffect::none \
	}
#define BOW_ATTACK_PRECISION_SHOT \
	{ \
		"Precision Shot", AttackType::ranged, AttackTargetType::single, (int)(0.7 * level + 3), penetration, 0.8, 0.3, \
		    3.0, 6, AttackEffect::none \
	}
#define BOW_ATTACK_TRIP_ARROW \
	{ \
		"Trip Arrow", AttackType::ranged, AttackTargetType::single, (int)(0.7 * level + 3), penetration, 0.95, 0.0, \
		    0.0, 4, AttackEffect::slow \
	}
#define BOW_ATTACK_HAIL_OF_ARROWS \
	{ \
		"Hail of Arrows", AttackType::ranged, AttackTargetType::multi, (int)(0.7 * level + 3), \
...
Equipment GenerateBow(int level, EquipmentMaterial material)
{
	...
	case EquipmentMaterial::wood:
		...
		penetration = (r_factor >= 4 && r_factor < 8) ? (std::rand() % level) + level / 2
		                                              : 0.5 * ((std::rand() % level) + level / 2);
	...
	case EquipmentMaterial::metal:
		agility = (r_factor < 4) ? ((std::rand() % level) + level / 2) : 0;
		perception = (r_factor >= 4 && r_factor < 8) ? 2 * ((std::rand() % level) + level / 2)
		                                             : (std::rand() % level) + level / 2;
	...
	case EquipmentMaterial::gold:
		agility = (r_factor < 4) ? 2 * ((std::rand() % level) + level / 2) : (std::rand() % level) + level / 2;
		perception = (r_factor >= 4 && r_factor < 8) ? 3 * ((std::rand() % level) + level / 2)
...
#define SHIELD_ATTACK_SHIELD_BASH \
	{ \
		"Shield Bash", AttackType::melee, AttackTargetType::single, (int)(0.5 * level + 1), 0, 0.85, 0.0, 0.0, 4, \
		    AttackEffect::slow \
	}
