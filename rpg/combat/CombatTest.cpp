...
#define RAT_STATS {2*lvl, 2*lvl, (int)(0.5*lvl + 1), 2*lvl, (int)(0.5*lvl + 1), lvl}
...
#define EQ_SWORD_ATTACK_SWORD_SWING {"Sword Swing", AttackType::melee, AttackTargetType::single, 9, 0.85, 0.3, 1.5, 6, AttackEffect::none}
#define EQ_BOW_ATTACK_PRECISION_SHOT {"Precision Shot", AttackType::ranged, AttackTargetType::single, 12, 0.8, 0.2, 3.0, 7, AttackEffect::none}
#define EQ_BOW_ATTACK_HAIL_OF_ARROWS {"Hail of Arrows", AttackType::ranged, AttackTargetType::multi, 6, 0.5, 0.05, 3.0, 9, AttackEffect::none}
...
{
    Equipment sword = Equipment(name, EquipmentType::sword, item_level, 0, (int)(0.5*item_level), 0,0,0,0);
...
{
    Equipment armor = Equipment(name, EquipmentType::armor, item_level,0,0,item_level,0,0,0);
    return armor;
}
...
int main(int argc, char* argv[])
{
    ...
    PlayerCombatant pc_hiei = PlayerCombatant("Hiei", 8, 80, 15, 13, 8, 12, 2, 9);
    pc_hiei.AddEquipment(GenerateSimpleSword("Metal Sword", 7));
    pc_hiei.AddEquipment(GenerateSimpleArmor("Plot Armor", 5));
    PlayerCombatant pc_pokkle = PlayerCombatant("Pokkle", 7, 60, 8, 4, 11, 4, 17, 4);
