...
class Equipment {
    public:
        Equipment(const std::string &name, equipment_type eq_type, int min_level, int agi, int str, int def, int dex,
                     int perc, int luck);
        ~Equipment();
        ...
        std::vector<Attacks> attacks() const {return attacks_;}
        ...
        int bonux_dexterity() const {return bonus_dexterity_;}
