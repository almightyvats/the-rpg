...
Vector2D GetCombatantPosition(bool player_team, int number)
{
	if (player_team) {
		switch (number) {
		case 0:
			return Vector2D(288, 250);
		case 1:
			return Vector2D(163, 150);
		case 2:
			return Vector2D(38, 250);
		default:
			return Vector2D(38, 50);
		}
	} else {
		switch (number) {
		case 0:
			return Vector2D(537, 250);
		case 1:
			return Vector2D(662, 150);
		case 2:
			return Vector2D(787, 250);
		case 3:
			return Vector2D(912, 150);
		default:
			return Vector2D(912, 350);
		}
	}
}
...
void RpgCombatState::GenerateCombat(std::vector<Combatant *> player_combatants, CombatArena arena)
{
	...
	for (Combatant *player_combatant : combat.player_combatants_) {
		...
		auto l_lvl = RpgLabel(pos.x + 50, pos.y - 35, "Lvl. " + std::to_string(player_combatant->level()), FONT_CLVL,
		                      color_white);
		...
		auto l_hp =
		    RpgLabel(pos.x + 12, pos.y + 135, "HP: " + std::to_string(player_combatant->hp()), FONT_CHP, color_white);
	...
	for (Combatant *enemy_combatant : combat.enemy_combatants_) {
		...
		l_name.CenterLabelHorizontally();
		auto l_lvl = RpgLabel(pos.x + 50, pos.y - 35, "Lvl. " + std::to_string(enemy_combatant->level()), FONT_CLVL,
		                      color_white);
		l_lvl.CenterLabelHorizontally();
		...
		auto l_cooldown = RpgLabel(pos.x + 12, pos.y + 170, "", FONT_CCD, color_white);
		auto l_hp =
		    RpgLabel(pos.x + 12, pos.y + 135, "HP: " + std::to_string(enemy_combatant->hp()), FONT_CHP, color_white);
		CombatantLabels l_combatant = {l_name, l_lvl, l_state, l_cooldown, l_hp};
		this->labels_combatants.push_back(l_combatant);
	}
	...
	if (SDL_PollEvent(&event) == 1) {
		...
		case SDL_KEYUP:
			if (combat.state() == CombatState::loot_display || combat.state() == CombatState::escape_screen
			    || combat.state() == CombatState::losing_screen) {
		...
		case SDL_MOUSEBUTTONDOWN:
			if (combat.state() == CombatState::action_selection
			    || combat.state() == CombatState::attack_target_selection
			    || combat.state() == CombatState::ability_target_selection || combat.state() == CombatState::start) {
...
void RpgCombatState::ProcessLabelClick(int index)
{
	...
	} else if (combat.state() == CombatState::attack_target_selection
	           || combat.state() == CombatState::ability_target_selection) {
...
void RpgCombatState::Update(RpgGame *rpgGame)
{
	...
	case CombatState::action_display:
		...
		if (combat.active_turn_chosen_attack() != NULL) {
			label_msg.setLabelText(FONT_MSG, combat.active_combatant()->name() + " uses "
			                                     + combat.active_turn_chosen_attack()->name);
			...
			label_msg.setLabelText(FONT_MSG, combat.active_combatant()->name() + " uses "
			                                     + combat.active_turn_chosen_ability()->name);
	...
	case CombatState::loot_display:
		label_msg.setLabelText(FONT_MSG, "You found a " + combat.loot().name() + " (Lvl. "
		                                     + std::to_string(combat.loot().min_level()) + ")");
		break;
	default:
		break;
	}
	...
	for (auto player_combatant : combat.player_combatants_) {
		...
		if (player_combatant->state_ != CombatantState::normal && player_combatant->state_ != CombatantState::dead) {
			l_combatant->state.setLabelText(FONT_CSTATE, player_combatant->state_string() + " ("
			                                                 + std::to_string(player_combatant->state_reset_countdown_)
			                                                 + ")");
		} else {
			l_combatant->state.setLabelText(FONT_CSTATE, player_combatant->state_string());
		}
	...
	for (auto enemy_combatant : combat.enemy_combatants_) {
		...
		if (enemy_combatant->state_ != CombatantState::normal && enemy_combatant->state_ != CombatantState::dead) {
			l_combatant->state.setLabelText(FONT_CSTATE, enemy_combatant->state_string() + "("
			                                                 + std::to_string(enemy_combatant->state_reset_countdown_)
			                                                 + ")");
		} else {
			l_combatant->state.setLabelText(FONT_CSTATE, enemy_combatant->state_string());
		}
...
void RpgCombatState::ConstructSelectionMenu(std::vector<std::string> element_labels)
{
	...
	for (std::string label : element_labels) {
		this->labels_selection.push_back(
		    RpgLabel(400 + (200 * column), 500 + (50 * row++), label, FONT_SELECTION, color_white));
